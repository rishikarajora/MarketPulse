...
const snapshot = {
  ...
  console.log('WATCHLIST:', latestWatchlist);
...
export function getDashboard() {
  ...
  return delay({
    ...
    sinceYouLastChecked: clone(changes),
...
export function getWatchlist() {
  const rows = stocks.filter((s) => currentWatchlist.includes(s.symbol));
...
export function addToWatchlist(symbol) {
  if (!currentWatchlist.includes(symbol)) {
    currentWatchlist = [...currentWatchlist, symbol];
